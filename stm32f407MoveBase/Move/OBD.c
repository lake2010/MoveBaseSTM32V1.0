...
void COBD_getElectricity(CODB_parameter* CODB_para)
{
	...
	val=val / (float)4095.00 * (float)3.30;
	CODB_para->m_electricity=(float)25/(float)1.65*(val-1.65);
...
float COBD_getVol(void)
{
	float val = 0;
	val = (float)m_volCount;
	m_volCount = 0;
	return val;
}
...
void COBD_loop(CODB_parameter* CODB_para)
{
	...
	if( millis() - CODB_para->m_volGetTime >= 100 ){
		...
		CODB_para->m_voltage += COBD_getVol();
	}
	...
	if( millis() - CODB_para->m_systemTime > 1000 ){//5s输出一次obd信息
		...
		if( CODB_para->m_getTimpReady ){
			...
			//发送COBD状态
			COBD_printfEncoding(temp_voltage,temp_batteryPercentage,temp_electricity
								,temp1_m_tmp,temp2_m_tmp,temp3_m_tmp);
			CODB_para->m_systemTime = millis();
			CODB_para->m_volReadCount = 0;
